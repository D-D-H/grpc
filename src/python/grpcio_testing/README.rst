...
``grpcio-testing`` is a pure-Python package; its free-threading
...
importing ``grpc_testing`` on a free-threaded interpreter does **not**
trigger CPython's automatic re-enablement of the GIL.

Support status:

* **Beta.** The testing utilities are expected to work, but
