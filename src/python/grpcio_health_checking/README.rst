...
``grpcio-health-checking`` is a pure-Python package; its free-threading
...
importing ``grpc_health`` on a free-threaded interpreter does **not**
trigger CPython's automatic re-enablement of the GIL.

Support status:

* **Beta.** The health-checking servicer is expected to work, but
