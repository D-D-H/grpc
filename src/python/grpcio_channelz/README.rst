...
``grpcio-channelz`` is a pure-Python package; its free-threading
...
importing ``grpc_channelz`` on a free-threaded interpreter does
**not** trigger CPython's automatic re-enablement of the GIL.

Support status:

* **Beta.** The channelz servicer is expected to work, but
