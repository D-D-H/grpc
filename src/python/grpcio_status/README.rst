...
``grpcio-status`` is a pure-Python package; its free-threading
...
importing ``grpc_status`` on a free-threaded interpreter does **not**
trigger CPython's automatic re-enablement of the GIL.

Support status:

* **Beta.** The status-proto helpers are expected to work, but
